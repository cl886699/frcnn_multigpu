...
keras_applications._KERAS_BACKEND = K
keras_applications._KERAS_LAYERS = KL
keras_applications._KERAS_MODELS = KM
keras_applications._KERAS_UTILS = KU
...
def block1(x, filters, kernel_size=3, stride=1,
    ...
    if conv_shortcut is True:
        ...
        shortcut = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                             name=name + '_0_bn')(shortcut, training=TRAIN_BN)
    else:
        shortcut = x

    x = layers.Conv2D(filters, 1, strides=stride, name=name + '_1_conv')(x)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_1_bn')(x, training=TRAIN_BN)
    x = layers.Activation('relu', name=name + '_1_relu')(x)

    x = layers.Conv2D(filters, kernel_size, padding='SAME',
                      name=name + '_2_conv')(x)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_2_bn')(x, training=TRAIN_BN)
    x = layers.Activation('relu', name=name + '_2_relu')(x)

    x = layers.Conv2D(4 * filters, 1, name=name + '_3_conv')(x)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_3_bn')(x, training=TRAIN_BN)
...
def block2(x, filters, kernel_size=3, stride=1,
    ...
    preact = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                       name=name + '_preact_bn')(x, training=TRAIN_BN)
...
def ResNet(stack_fn,
    ...
    if preact is False:
        x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                      name='conv1_bn')(x, training=TRAIN_BN)
    ...
    if preact is True:
        x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                      name='post_bn')(x, training=TRAIN_BN)
...
if __name__ == "__main__":
    ...
    n_classes = 1000
    fc_activation = 'sigmoid'  # softmax sigmoid
    print('model_name', model_name)
    model = ResNeXt101(weights=None, input_shape=input_shape, classes=n_classes)
    # model = ResNet101(False,input_shape=input_shape)
    model.summary()
    print('-' * 10)

    # from tensorflow.keras.utils import plot_model
    #
    # plot_model(model, to_file=r'E:\model\resnext101.pdf', show_shapes=True, expand_nested=True, dpi=300)