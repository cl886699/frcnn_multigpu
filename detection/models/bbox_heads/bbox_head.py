...
class BBoxHead(tf.keras.Model):
    ...
    def get_bboxes(self, rcnn_probs, rcnn_deltas, rois, img_metas):
        ...
        detections_list = [
            self._get_bboxes_single(
                rcnn_probs[i], rcnn_deltas[i], rois[i], img_metas[i][0:2])
    ...
    def _get_bboxes_single(self, rcnn_probs, rcnn_deltas, rois, img_shape):
        ...
        refined_rois *= tf.cast(tf.repeat(img_shape, 2), tf.float32)
        window = tf.concat([tf.constant([0., 0.], tf.float32), tf.cast(img_shape, tf.float32)], axis=0)
