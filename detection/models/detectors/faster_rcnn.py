...
from detection.models.backbones import resnet,resnext
...
class FasterRCNN(tf.keras.Model, RPNTestMixin, BBoxTestMixin):
    def __init__(self, num_classes, **kwags):
        ...
        # RPN configuration
        # Anchor attributes
        self.ANCHOR_SCALES = (8, 13, 32, 64, 128)
        ...
        # Modules
        # self.backbone = resnext.ResNet101(False)
    ...
    def call(self, inputs, training=True):      
        if training: # training
            imgs, imgs_shape, gt_boxes, gt_class_ids = inputs
        else: # inference
            imgs, imgs_shape = inputs
        ...
        if training:
            rpn_class_loss, rpn_bbox_loss = self.rpn_head.loss(
                ...
                gt_boxes, gt_class_ids, imgs_shape)
            ...
            detections_list = self.bbox_head.get_bboxes(
                rcnn_probs, rcnn_deltas, rois, imgs_shape)
...
if __name__ == '__main__':
    total_categaries = 16
    one_imgs = tf.expand_dims(tf.random.uniform([512,512,3], dtype=tf.float32), axis=0)
    one_metas = tf.expand_dims(tf.constant([512,512,3], tf.float32),axis=0)
    print(one_imgs)
    print(one_metas)
    model = FasterRCNN(
        num_classes=17)
