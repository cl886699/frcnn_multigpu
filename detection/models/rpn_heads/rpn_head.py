...
class RPNHead(tf.keras.Model):
    ...
    def loss(self, rpn_class_logits, rpn_deltas, gt_boxes, gt_class_ids, imgs_shape):
        '''Calculate rpn loss
        '''
        anchors = self.generator.generate_pyramid_anchors(imgs_shape)
        
        rpn_labels, rpn_label_weights, rpn_delta_targets, rpn_delta_weights = \
            self.anchor_target.build_targets(anchors, gt_boxes, gt_class_ids)
    ...
    def get_proposals(self, 
        ...
        '''
        anchors = self.generator.generate_pyramid_anchors(img_shape)
        
        rpn_probs = rpn_probs[:, :, 1]
        
        proposals_list = [
            self._get_proposals_single(
                rpn_probs[i], rpn_deltas[i], anchors, img_shape[i, 0:2], i, with_probs)
    ...
    def _get_proposals_single(self, 
        ...
        window = tf.concat([tf.constant([0., 0.], dtype=tf.float32), tf.cast(img_shape, tf.float32)], axis=0)
